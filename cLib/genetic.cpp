...
void Solution::metamodel()
{
    ...
    holdingCost = std::max(holdingCost,0.0);
    shortageCost = std::max(shortageCost,0.0);
...
bool Genetic::x1(Solution &s1, Solution &s2)
{
    ...
    //std::cout << hp.slackTreshold << std::endl;
...
void Genetic::reproduction(std::vector<Solution> &winners)
{
    ...
    while (sons < childrenNumber)
    {
        ...
        Params son1, son2;
        double alpha = -0.2 + static_cast<float>(rand()) / (static_cast<float>(RAND_MAX / 1.4));
